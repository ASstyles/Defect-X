...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await signupWithEmail(auth, email, password, fullName);
      toast({
        title: 'Account Created',
        description: 'Welcome to Defect X! Successfully registered.',
      });
      router.push('/');
    } catch (error: any) {
      toast({
  ...
  const triggerBypass = async () => {
    ...
    try {
      if (!auth) throw new Error('Firebase Auth not initialized');
      await signupWithEmail(auth, 'operator.alpha@defectx.ai', 'bypasspassword123', 'Alpha Operator');
      toast({
        title: 'Bypass Successful',
        description: 'Registered and logged in with mock administrator profile.',
      });
      router.push('/');
    } catch (error: any) {
      toast({
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-amber-400/70 leading-relaxed text-[11px]">
                    No active Firebase keys found. Using the local bypass system. Create any operator account here.
                  </p>
                </div>
              </div>
              <Button 
            ...
            <form onSubmit={handleSignup} className="space-y-5 relative z-10">
              <div className="space-y-1 text-center pb-3">
                <h2 className="text-[48px] font-bold font-headline leading-none tracking-tight text-white select-none flex flex-col gap-1.5">
                  <span className="text-xl font-normal tracking-wide text-zinc-400">Create</span>
                  <span className="flex items-center justify-center gap-1 pb-1">
                    <span className="bg-gradient-to-b from-white via-zinc-100 to-zinc-400 bg-clip-text text-transparent filter drop-shadow-[0_0_15px_rgba(255,255,255,0.15)]">Account</span>
                  </span>
                </h2>
                <p className="text-zinc-400 text-xs font-normal">
                  Register a new operator credential profile
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <div className="relative group/input">
                    <User className="absolute left-3.5 top-3.5 h-4 w-4 text-zinc-500 group-focus-within/input:text-sky-400 transition-colors pointer-events-none" />
                    <Input 
                      id="name" 
                      ...
                      className="pl-10 h-11 custom-input text-white placeholder:text-zinc-650 rounded-lg focus-visible:ring-0 focus-visible:ring-offset-0"
                      disabled={submitting}
                      required
                    />
                  </div>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="password" className="text-zinc-400 text-xs font-medium">Password (min. 6 chars)</Label>
                  <div className="relative group/input">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="confirmPassword" className="text-zinc-400 text-xs font-medium">Confirm Password</Label>
                  <div className="relative group/input">
                    <Lock className="absolute left-3.5 top-3.5 h-4 w-4 text-zinc-500 group-focus-within/input:text-sky-400 transition-colors pointer-events-none" />
                    <Input 
                      id="confirmPassword" 
              ...
              <p className="text-center text-xs text-zinc-500 pt-1">
                ...
                <Link href="/login" className="text-sky-400 hover:text-[#58cbfb] hover:underline font-semibold transition-colors">
                  Sign in
                </Link>
              </p>
